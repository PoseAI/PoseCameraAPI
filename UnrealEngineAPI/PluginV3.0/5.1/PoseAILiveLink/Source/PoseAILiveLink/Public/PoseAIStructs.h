...
public:
    ...
    UPROPERTY()
        TArray<float> Hip;
    UPROPERTY()
        TArray<float> FootIkL;
    UPROPERTY()
        TArray<float> FootIkR;
};
...
public:
    ...
    /** location of subject in camera frame, scaled in body units (i.e. multiply by rig height to translate to game world). Pos Y moves toward camera */
    UPROPERTY(BlueprintReadOnly, Category = "PoseAI")
    FVector rootTranslation = FVector(0.0f, -2.0f, 0.0f);
    ...
    /** DEPR current height of jump in body units */
    UPROPERTY(BlueprintReadOnly, Category = "PoseAI_DEPR")
    float jumpHeight = 0.0f;

    /** DEPR estimated actual height of the subject in clip coordinates (2.0 = full height of image) */
    UPROPERTY(BlueprintReadOnly, Category="PoseAI_DEPR")
    float bodyHeight = 0.0f;
    ...
    /** location of left thumb tip in camera frame (clip coordinates -1 to 1, 0 is center ) */
    UPROPERTY(BlueprintReadOnly, Category = "PoseAI")
    FVector2D pointThumbLeft = FVector2D(0.0f, 0.0f);

    /** location of right thumb tip in camera frame (clip coordinates -1 to 1, 0 is center ) */
    UPROPERTY(BlueprintReadOnly, Category = "PoseAI")
    FVector2D pointThumbRight = FVector2D(0.0f, 0.0f);

    /** how open the left hand is currently */
    UPROPERTY(BlueprintReadOnly, Category = "PoseAI")
    float opennessLeftHand = 0.5f;

    /** how open the right hand is currently */
    UPROPERTY(BlueprintReadOnly, Category = "PoseAI")
    float opennessRightHand = 0.5f;
    ...
    /** target for the PoseAI Foot IK node */
    UPROPERTY(BlueprintReadOnly, Category = "PoseAI")
        FVector footIkL = FVector(0.0f, 0.0f, 0.0f);

    /** target for the PoseAI Foot IK node */
    UPROPERTY(BlueprintReadOnly, Category = "PoseAI")
        FVector footIkR = FVector(0.0f, 0.0f, 0.0f);
    ...
    FVector rootOffset = FVector(0.0f, 0.0f, 0.0f);
    FRotator cameraRotation = FRotator(0.0f, 0.0f, 0.0f);
    FVector scaleMotion = FVector(1.0f, 0.0f, 1.0f);
    ...
    void ProcessVerboseVectorsHandRight(const TSharedPtr < FJsonObject > vecHand);
    void ProcessCompactScalarsBody(const FString& compactString);
    void ProcessCompactVectorsBody(const FString& compactString);
    void ProcessCompactVectorsHandLeft(const TSharedPtr < FJsonObject >);
    void ProcessCompactVectorsHandRight(const TSharedPtr < FJsonObject >);

private:
    static const FString fieldPointScreen;
    static const FString fieldThumbScreen;
