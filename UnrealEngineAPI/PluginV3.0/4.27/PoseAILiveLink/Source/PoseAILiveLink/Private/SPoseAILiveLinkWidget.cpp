...
// right now this is manually aligned with the enums but should be a lookup to keep it from breaking
static TArray<FString> PoseAI_Modes = { "Room", "Desktop", "Portrait", "RoomBodyOnly", "PortraitBodyOnly" };
static TArray<FString> PoseAI_Rigs = { "MetaHuman", "UE4", "Mixamo", "DazUE"};   

const FString SPoseAILiveLinkWidget::section = "PoseLiveLink.SourceConfig";
int32 SPoseAILiveLinkWidget::portNum = PoseAILiveLinkNetworkSource::portDefault;
...
FPoseAIHandshake SPoseAILiveLinkWidget::GetHandshake()
{
	FPoseAIHandshake handshake = FPoseAIHandshake();
	handshake.isMirrored = isMirrored;
	handshake.rig = static_cast<EPoseAiRigPresets>(rigIndex);
	handshake.mode = static_cast<EPoseAiAppModes>(modeIndex);
...
{
	return PoseAILiveLinkNetworkSource::MakeSource(GetHandshake(), portNum, isIPv6);
}
