...
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FPoseAIResetLivePositionEvent);
...
class POSEAILIVELINK_API UPoseAIMovementComponent : public UActorComponent
{
 ...
 public:
     ...
     /** sends disconnect message to app and closes source, freeing up Port*/
     UFUNCTION(BlueprintCallable, Category = "PoseAI Setup")
     void CloseSource();
     ...
     /** Sets rig height, which scales root motion, and allows scaling per dimension (i.e. set Y=0 for no motion to/from camera) */
     UFUNCTION(BlueprintCallable, Category = "PoseAI Configuration")
     void ScaleMotion(float RigHeight=170.0f, FVector Scale=FVector(1.0f, 1.0f,1.0f));
     
     /** compensates for an active source's camera rotation in the real world */
     UFUNCTION(BlueprintCallable, Category = "PoseAI Configuration")
     void SetLiveCameraRotation(float pitch, float yaw = 0.0f, float roll=0.0f);
     
     UFUNCTION(BlueprintCallable, Category = "PoseAI Configuration")
     void UseCurrentPoseAsBaseTranslation();
     
     /** Have player stand up straight and face screen as part of configuration */
     UFUNCTION(BlueprintCallable, Category = "PoseAI Configuration")
     void UseCurrentPoseToOrientCamera();

     /** Remove all live root motion (sets scalemotion to zero)*/
     UFUNCTION(BlueprintCallable, Category = "PoseAI Configuration")
         void ZeroMotion();
...
public:
    ...
    /** Convenience event to tell animation blueprin*/
    UFUNCTION(BlueprintCallable, Category = "PoseAI Events")
    void BroadcastResetLivePosition();
    ...
    UPROPERTY(BlueprintAssignable, Category = "PoseAI Events")
    FPoseAIResetLivePositionEvent resetLivePositionEvent;
    ...
    void BroadcastResetZeroLivePosition();
