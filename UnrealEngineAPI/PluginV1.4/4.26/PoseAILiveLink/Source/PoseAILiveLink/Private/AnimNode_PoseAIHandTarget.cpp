...
void FAnimNode_PoseAIHandTarget::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	const FVector Bone1CSPos = Output.Pose.GetComponentSpaceTransform(SpineFirstIndex).GetTranslation();
	const FVector Bone2CSPos = Output.Pose.GetComponentSpaceTransform(LeftUpperArmIndex).GetTranslation();
	const FVector Bone3CSPos = Output.Pose.GetComponentSpaceTransform(RightUpperArmIndex).GetTranslation();
	const FVector Bone4CSPos = Bone1CSPos + 0.5f * (FVector::Dist(Bone2CSPos, Bone1CSPos) + FVector::Dist(Bone3CSPos, Bone1CSPos)) * 
		FVector::CrossProduct(Bone3CSPos - Bone1CSPos, Bone2CSPos - Bone1CSPos).GetSafeNormal();
	FVector TargetLocation = 
		PoseAiIkVector.X * Bone1CSPos + 
		PoseAiIkVector.Y * Bone2CSPos + 
		PoseAiIkVector.Z * Bone3CSPos +
		(1.0f - PoseAiIkVector.X - PoseAiIkVector.Y - PoseAiIkVector.Z) * Bone4CSPos;
