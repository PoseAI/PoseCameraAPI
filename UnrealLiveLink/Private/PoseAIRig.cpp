...
bool PoseAIRig::ProcessFrame(const TSharedPtr<FJsonObject> jsonObject, FLiveLinkFrameDataStruct& frameData)
{
	...
	if (rotBody == nullptr && cachedPose.Num() < 1) {
		...
		// set the appropriate height of the root and hip/pelvis
		...
		if (scaBody != nullptr) {
			...
			data.Transforms[kPoseAILeftShoulderJoint].AddToTranslation(leftShrug * shrugVector);
			(*scaBody)->TryGetNumberField(fieldShrugRight, rightShrug);
			data.Transforms[kPoseAIRightShoulderJoint].AddToTranslation(rightShrug * shrugVector);
		}

		// calculates lowest Z in component space.  checks first 22 joints to save calculations on fingers: if this is important consider using parents.Num()
		...
		float minZ = 0.0f;
		for (int32 j = 2; j < (includeHands ? 23 : 21) ; j++) {
		...
		//update adjustment only when stable
...
FLiveLinkStaticDataStruct PoseAIRig::MakeUE4Rig()
{
...
FLiveLinkStaticDataStruct PoseAIRig::MakeMixamoRig()
{
	FLiveLinkStaticDataStruct staticData;
	staticData.InitializeWith(FLiveLinkSkeletonStaticData::StaticStruct(), nullptr);
	FLiveLinkSkeletonStaticData* skelData = staticData.Cast<FLiveLinkSkeletonStaticData>();
